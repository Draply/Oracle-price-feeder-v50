...
func priceFeederCmdHandler(cmd *cobra.Command, args []string) error {
	...
	// Set address prefixes based on configuration
	sdkConfig := sdk.GetConfig()
	sdkConfig.SetBech32PrefixForAccount(cfg.Account.Prefix, cfg.Account.Prefix+"pub")
	sdkConfig.SetBech32PrefixForValidator(cfg.Account.Prefix+"valoper", cfg.Account.Prefix+"valoperpub")
	sdkConfig.SetBech32PrefixForConsensusNode(cfg.Account.Prefix+"valcons", cfg.Account.Prefix+"valconspub")
	sdkConfig.Seal()
	...
	for denom, weights := range cfg.ProviderWeights {
		...
		for providerName, value := range weights {
			...
			value, err := math.LegacyNewDecFromStr(fmt.Sprintf("%f", value))
			if err != nil {
				return err
			}
