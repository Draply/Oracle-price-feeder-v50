...
func (p *UniswapV3Provider) Poll() error {
	...
	for symbol, pair := range p.getAllPairs() {
		...
		price, err := math.LegacyNewDecFromStr(sqrtx96.Pow(two).Div(two.Pow(decimal.NewFromInt(192))).String())
		if err != nil {
		...
		if decimalsBase >= decimalsQuote {
			...
			price = price.Mul(math.LegacyNewDec(10).Power(diff))
		} else {
			diff = decimalsQuote - decimalsBase
			price = price.Quo(math.LegacyNewDec(10).Power(diff))
		}
