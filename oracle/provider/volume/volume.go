...
type Volume struct {
	Height uint64
	Time   int64
	Values map[string]math.LegacyDec
}
...
func (h *VolumeHandler) load() error {
	...
	for rows.Next() {
		...
		volume, err := math.LegacyNewDecFromStr(value)
		if err != nil {
			h.logger.Err(err).Msg("")
			return err
		}
...
func (h *VolumeHandler) Get(symbol string) (math.LegacyDec, error) {
...
func (h *VolumeHandler) Add(volumes []Volume) {
	...
	blocks := uint64(round.Round(float64(first.Time-startTime) / blockTime))
