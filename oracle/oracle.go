...
import (
	...
	oracletypes "appchain/x/oracle/types"
...
type ProviderWeight struct {
	Type   string
	Weight map[string]math.LegacyDec
}
...
func New(
	...
	return &Oracle{
		...
		bypassOracleParams:   bypassOracleParams,
	}
}
...
func GetComputedPrices(
	...
	providerMinOverrides map[string]int,
	providerWeights map[string]ProviderWeight,
) (prices map[string]math.LegacyDec, err error) {
...
func (o *Oracle) GetParams(ctx context.Context) (oracletypes.Params, error) {
	o.logger.Debug().
		Str("grpc_endpoint", o.oracleClient.GRPCEndpoint).
		Msg("connecting to gRPC to query oracle parameters")
	...
	if err != nil {
		o.logger.Error().
			Err(err).
			Str("grpc_endpoint", o.oracleClient.GRPCEndpoint).
			Msg("failed to get x/oracle params from gRPC")
		return oracletypes.Params{}, fmt.Errorf("failed to get x/oracle params: %w", err)
	}

	o.logger.Debug().
		Uint64("vote_period", queryResponse.Params.VotePeriod).
		Int("whitelist_count", len(queryResponse.Params.AcceptList)).
		Msg("successfully retrieved oracle parameters from blockchain")
...
func (o *Oracle) tick(ctx context.Context) error {
	...
	if len(o.priceProviders) == 0 {
		o.logger.Debug().Msg("no price providers found, initializing SetPrices")
		o.SetPrices(ctx)
	}

	blockHeight, err := o.oracleClient.ChainHeight.GetChainHeight()
	if err != nil {
		o.logger.Error().Err(err).Msg("failed to get chain height")
		return err
	}
	if blockHeight < 1 {
		o.logger.Error().Int64("block_height", blockHeight).Msg("expected positive block height")
	...
	o.logger.Debug().
		...
		Str("grpc_endpoint", o.oracleClient.GRPCEndpoint).
		Str("tmrpc_endpoint", o.oracleClient.TMRPC).
		Msg("querying oracle parameters")

	oracleParams, err := o.GetParamCache(ctx, blockHeight)
	if err != nil {
		o.logger.Error().Err(err).Msg("failed to get oracle parameters")
		return err
	}

	o.logger.Debug().
		Uint64("vote_period", oracleParams.VotePeriod).
		Int("whitelist_count", len(oracleParams.AcceptList)).
		Msg("oracle parameters retrieved")

	// Log whitelist denoms
	for i, denom := range oracleParams.AcceptList {
		o.logger.Debug().
			Int("index", i).
			Str("denom", denom.SymbolDenom).
			Msg("whitelist denom")
	}
	...
	currentVotePeriod := math1.Floor(float64(nextBlockHeight) / float64(oracleVotePeriod))
	...
	o.logger.Debug().
		...
		Int64("next_block_height", nextBlockHeight).
		Str("grpc_endpoint", o.oracleClient.GRPCEndpoint).
		Msg("oracle tick debug info")

	// Skip until new voting period. Specifically, skip when:
	// index [0, oracleVotePeriod - 1] > oracleVotePeriod - 2 OR index is 0
	skipCondition1 := o.previousVotePeriod != 0 && currentVotePeriod == o.previousVotePeriod
	skipCondition2 := indexInVotePeriod > 0 && oracleVotePeriod-indexInVotePeriod > 2

	o.logger.Debug().
		Bool("skip_condition1", skipCondition1).
		Bool("skip_condition2", skipCondition2).
		Int64("oracleVotePeriod_minus_indexInVotePeriod", oracleVotePeriod-indexInVotePeriod).
		Msg("voting skip conditions check")

	if skipCondition1 || skipCondition2 {
		// oracleVotePeriod-indexInVotePeriod < 2 || (indexInVotePeriod > 0 && indexInVotePeriod < int64(float64(oracleVotePeriod)*0.75)) {
		o.logger.Info().
			Float64("previous_vote_period", o.previousVotePeriod).
			Float64("current_vote_period", currentVotePeriod).
			Int64("indexInVotePeriod", indexInVotePeriod).
			Int64("oracleVotePeriod", oracleVotePeriod).
			Bool("skip_condition1", skipCondition1).
			Bool("skip_condition2", skipCondition2).
	...
	o.logger.Debug().Msg("setting prices from providers")
	if err := o.SetPrices(ctx); err != nil {
		o.logger.Error().Err(err).Msg("failed to set prices")
		return err
	}

	// Log current prices
	currentPrices := o.GetPrices()
	o.logger.Debug().
		Int("price_count", len(currentPrices)).
		Str("prices", currentPrices.String()).
		Msg("current oracle prices")
	...
	if o.previousVotePeriod != 0 && currentVotePeriod-o.previousVotePeriod != 1 {
		o.logger.Info().
			Float64("previous_vote_period", o.previousVotePeriod).
			Float64("current_vote_period", currentVotePeriod).
			Float64("period_difference", currentVotePeriod-o.previousVotePeriod).
	...
	if err != nil {
		o.logger.Error().Err(err).Msg("failed to generate salt")
		return err
	}

	o.logger.Debug().
		Str("salt", salt).
		Msg("generated salt for vote")

	valAddr, err := sdk.ValAddressFromBech32(o.oracleClient.ValidatorAddrString)
	if err != nil {
		o.logger.Error().Err(err).Str("validator", o.oracleClient.ValidatorAddrString).Msg("failed to parse validator address")
		return err
	}
	...
	o.logger.Debug().
		Str("exchange_rates", exchangeRatesStr).
		Str("hash", hash.String()).
		Str("validator", valAddr.String()).
		Msg("generated vote hash")
	...
	o.logger.Debug().
		Bool("isPrevoteOnlyTx", isPrevoteOnlyTx).
		Interface("previousPrevote", o.previousPrevote).
		Msg("checking if this is prevote only transaction")
	...
	if isPrevoteOnlyTx {
		...
		o.logger.Info().
			...
			Int64("timeout_blocks", oracleVotePeriod*2).
			Msg("broadcasting pre-vote")
		if err := o.oracleClient.BroadcastTx(nextBlockHeight, oracleVotePeriod*2, preVoteMsg); err != nil {
			o.logger.Error().Err(err).Msg("failed to broadcast prevote")
			return err
		}

		currentHeight, err := o.oracleClient.ChainHeight.GetChainHeight()
		if err != nil {
			o.logger.Error().Err(err).Msg("failed to get current height after prevote")
			return err
		}

		o.previousVotePeriod = math1.Floor(float64(currentHeight) / float64(oracleVotePeriod))
		...
		o.logger.Debug().
			Str("salt", salt).
			Str("exchange_rates", exchangeRatesStr).
			Int64("submit_block_height", currentHeight).
			Float64("previous_vote_period", o.previousVotePeriod).
			Msg("prevote stored for next vote")
	} else {
		o.logger.Debug().
			Str("previous_salt", o.previousPrevote.Salt).
			Str("previous_exchange_rates", o.previousPrevote.ExchangeRates).
			Int64("previous_submit_block", o.previousPrevote.SubmitBlockHeight).
			Msg("sending vote message with previous prevote data")
		...
		o.logger.Info().
			...
			Str("salt", voteMsg.Salt).
			Int64("timeout_blocks", oracleVotePeriod-indexInVotePeriod).
		...
		if err := o.oracleClient.BroadcastTx(
			...
			o.logger.Error().Err(err).Msg("failed to broadcast vote")
			return err
		}
		...
		o.logger.Debug().
			...
			Str("previous_exchange_rates", o.previousPrevote.ExchangeRates).
			Int64("previous_submit_block", o.previousPrevote.SubmitBlockHeight).
			Msg("vote completed, clearing previous prevote")
...
func GenerateExchangeRatesString(prices sdk.DecCoins) string {
	...
	for _, price := range prices {
		pairs = append(pairs, fmt.Sprintf("%s:%s", price.Denom, price.Amount.String()))
	}
