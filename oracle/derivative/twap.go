...
func Twap(
...
) (math.LegacyDec, int64, error) {
	priceTotal := math.LegacyZeroDec()
	timeTotal := int64(0)
	...
	threshold, err := math.LegacyNewDecFromStr(fmt.Sprintf("%f", twapMaxPriceDeviation))
	if err != nil {
		return math.LegacyDec{}, 0, fmt.Errorf("failed to parse twapMaxPriceDeviation: %w", err)
	}
	...
	if median.IsZero() {
		return math.LegacyDec{}, 0, fmt.Errorf("median is 0")
	}
	...
	if timeTotal < minPeriod {
		...
		return math.LegacyDec{}, missing, fmt.Errorf(message)
	}

	return priceTotal.QuoInt64(timeTotal), 0, nil
}

func weightedMedian(tickers []types.TickerPrice) (math.LegacyDec, error) {
	type Price struct {
